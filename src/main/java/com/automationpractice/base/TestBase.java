...
public class TestBase {
	...
	public TestBase() {
		...
		try {
			FileInputStream ip = new FileInputStream("C:\\Users\\joyce\\eclipse-workspace\\shoppingcart\\src\\main\\java\\com\\automationpractice\\config\\config.properties");
			try {
				prop.load(ip);
			} catch (IOException e) {
		...
		new WebDriverWait(driver, secondsToWait).until((ExpectedCondition<Boolean>) wd -> {
			...
			while (true) {
				...
				if (readyState.equals("complete")) {
					...
					System.out.println("Document is not in ready Ready State :" + readyState);
				}
			}
